...
def filter_list_by_gender(person_list: list, gender: str) -> list:
    ...
    """
    return list(filter(lambda x: True if x.gender in gender or x.gender == "Undefined" else False, person_list))
...
def filter_list_by_age(person_list: list, bottom_age: int, upper_age: int) -> list:
    ...
    WRONG EXPLANATION, correct: between the bottom and upper_ages.
    ...
    :return: list of people
    """
    return list(filter(lambda x: True if bottom_age <= x.age <= upper_age else False, person_list))
...
def filter_list_by_bmi(person_list: list) -> list:
    ...
    :return: list of people
    """
    return list(filter(lambda x: True if 18.5 <= x.weight / (x.height / 100) ** 2 <= 25 else False, person_list))
...
def get_the_rating_product(person_list: list) -> int:
    ...
    :return: product
    """
    return reduce(lambda x, y: x * y, map(lambda x: x.rating, person_list))
...
def sort_by_name_length(person_list: list) -> list:
    ...
    """
    return sorted(person_list, key=lambda x: len(x.surname) + len(x.first_name), reverse=True)
...
def get_people_with_the_lowest_rating(person_list: list) -> list:
    ...
    :return: list of people
    """
    return list(filter(lambda x: True if x.rating == min(map(lambda y: y.rating, person_list)) else False, person_list))
